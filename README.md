...
- [结构图](#结构图)
- [📊 盘口指标提取](#-盘口指标提取)
- [📤 衍生品全量数据自动导出](#-衍生品全量数据自动导出)
- [💰 个股资金流明细自动归档](#-个股资金流明细自动归档)
- [跨流水线的共性问题](#跨流水线的共性问题)
- [环境说明](#环境说明)
- [合规声明](#️-合规声明)
...
| 📤 | **衍生品数据** | 每天自动导出 7 次，沪 / 深 / 中金所三大板块全部期权数据 | ⭐⭐⭐⭐ |
...
三条线**共用同一套原则**：不依赖界面结构、不假设坐标稳定、**宁可报错跳过，也绝不返回一个看起来合理的错数字**。
...
**先看 OCR 的实际表现**：图上明明是 `-19.2 / -28.9 / -40.1`，识别结果是 `空 / -7 / -7`——**12 个数据点，0 个对**。更麻烦的是 `-7` 落在当天的合理区间内，**扫一眼根本看不出是错的**。

**失败的直接原因出在喂给它的图上**：逐行统计读取区域的像素后发现，**框内混进了整条资金流进度条和分隔线**（14 行实心色块 + 满宽实线）。通用 OCR 按「单行文本」处理「数字 + 实心方块」，必然吐垃圾。

**然后发现这件事根本不需要「识别」**：终端数字用的是固定点阵字体，**所有数字都一个不差地落进这 12 种像素图案**（`0-9` + `.` + `-`）。

![点阵字形原理](assets/glyphs-12.png)

既然每种字符只有一种画法，逐像素比对就够了：

| | 通用 OCR | 点阵模板匹配 |
|---|---|---|
| 怎么得出结果 | 特征提取 + 模型推理 | 逐像素比对 |
| 会猜吗 | **会** ——「大概 87% 是 9」 | **不会** —— 中，或不中 |
| 额外依赖 | OCR 引擎 + 版本兼容 | 无（纯 Python 包） |

顺带把运行依赖从 4 个减到 3 个：不用装 Tesseract、不用起 Umi-OCR 进程、不用趟 PaddleOCR 的版本坑。

<!-- 字形位图 BEGIN -->
<details>
<summary><b>ⓘ 展开：12 种字形的原始位图数据</b></summary>

从源码里直接解出来的点阵（`#` = 亮像素，`·` = 暗像素）。数字统一 6×10，`1` 只占 4 像素宽；小数点 2×2，负号 4×1——**小数点与负号在纵向中部，不是顶格**。
...
> **关键收获**：**先问「这个数据有没有更近的源头」，再决定要不要截图。** 这个项目一开始假设"必须 OCR"，实际上终端自带导出、公开接口都能拿到同一份数据——**差点在一件本可以绕开的事情上花掉一半工期。**
...
| 阶段 | 目标 | 说明 |
|---|---|---|
| 原型 | 每 15 分钟导出自选股数据 | `PyAutoGUI` + `PyGetWindow`，手动测坐标 |
| 正式版 | 每日 **18 个时点** | 导出 → 加时间列 → 按日合并 → 邮件投递 |
...
**配套工具**：坐标采集器 —— 按空格记录鼠标坐标 + 实时 RGB，逐条追加写入文本文件，解决「频繁测量坐标、手抄容易错」的问题。

> **关键收获**：**能不用坐标就不用坐标。** 菜单项一多一少，固定坐标就会点偏；键盘导航（`End` / `↑` / `Enter`）比坐标**鲁棒一个数量级**，因为它是按**语义**定位、不是按**像素**定位。

---

## 跨流水线的共性问题

三条线数据源和技术栈完全不同，**踩的坑却高度同构**。

| 共性问题 | 📊 盘口 | 📤 衍生品 | 💰 资金流 | **通用解法** |
|---|---|---|---|---|
...
| 流水线 | 运行环境 |
|---|---|
| 📊 📤 💰 | Windows · Python 3.11 · Pillow / NumPy / OpenPyXL / PyAutoGUI · 国产 RPA 平台 |
