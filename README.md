...
![流水线](https://img.shields.io/badge/流水线-3条-2f81f7?style=for-the-badge)
![频次](https://img.shields.io/badge/日均采集-74个时点-f0883e?style=for-the-badge)
![可信度](https://img.shields.io/badge/数值可信度-99.7%25-3fb950?style=for-the-badge)
![交付](https://img.shields.io/badge/交付-单文件EXE-8957e5?style=for-the-badge)
...
| | **项目** | **它替你干什么** | **结果** |
|:--:|---|---|---|
| 📊 | **盘口指标** | 每天自动盯盘 **49** 次，记下涨跌家数与资金流向 | 数值可信度 **99.7%** |
| ⚙️ | **衍生品数据** | 每天自动导出 **7** 次三大板块全部期权数据 | **零人工干预** |
| 💰 | **资金流明细** | 每天自动整理 **18** 次，合并后邮件送到手上 | **全流程自动** |

> 加起来每天 **74 个采集时点**，全部开机自动跑，输出直接能用于分析建模。
> 三条线**按技术难度从高到低排列**——读者的注意力是递减的。
...
flowchart LR
    subgraph A["📊 盘口指标提取"]
        direction LR
        A1["行情终端"] -->|"定时截图<br/>49 时点/日"| A2["全屏 PNG"]
        A2 --> A3["布局指纹<br/>判窗口布局"]
        A3 --> A4["点阵模板匹配<br/>读数（零 OCR）"]
        A4 --> A5["跨帧一致性<br/>校验"]
    end

    subgraph B["⚙️ 衍生品全量导出"]
        direction LR
        B1["期权交易终端<br/>（自绘 UI）"] -->|"RPA 定时触发"| B2["元素捕获 +<br/>图像定位兜底"]
        B2 --> B3["三市场<br/>自动导出闭环"]
        B3 --> B4["按批次重命名<br/>加精确时间戳"]
    end

    subgraph C["💰 资金流明细归档"]
        direction LR
        C1["行情终端"] -->|"内置导出<br/>键盘导航选菜单"| C2["主力资金 /<br/>大资金明细"]
        C2 --> C3["自动识别表头<br/>加时间列"]
        C3 --> C4["按日合并<br/>→ 邮件投递"]
    end
    ...
    classDef store fill:#e8f4ff,stroke:#4a90d9,stroke-width:2px
    class DB store
```
